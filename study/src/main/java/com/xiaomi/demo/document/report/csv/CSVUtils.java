...
public class CSVUtils {
    ...
    public static <T> List<T> parseAsList(String[] headers, Reader reader, Class<T> itemClass) {
        try {
                ...
                fields.forEach(csvCellField -> {
                    try {
                        ...
                        if (csvCellField.isRequired()) {
                            throw new RuntimeException(StrUtil.format("Inject required field {} failed. row: {}", csvCellField.getHeader(), record.toString()), e);
                        }
                    }
                });
                list.add(row);
            ...
            throw new RuntimeException(StrUtil.format("Failed to parse CSV file as List<{}>", itemClass.getSimpleName()), e);
        } finally {
    ...
    private static Object parseNumber(CSVCellField csvCellField, String strValue) {
        try {
            ...
            String msg = StrUtil.format("invalid number {} for field {}", strValue, csvCellField.getField().getName());
    ...
    /**
     * csv导出，根据注解自动生成header
     *
     * @param dataSet
     * @param file
     * @param <T>
     */
    public static <T> void exportCSV(Class<?> clazz, Collection<T> dataSet, File file) {
        try {
            exportCSV(getCellHeaders(clazz), dataSet, new FileOutputStream(file));
        } catch (FileNotFoundException e) {
            throw new RuntimeException("export CSV failed", e);
        }
    }
     ...
     * @return
     */
    public static <T> byte[] exportCSVByteArray(Class<?> clazz, Collection<T> dataSet) {
        try {
            File file = File.createTempFile("FILE_CLIENT_", null);
            file.deleteOnExit();
            exportCSV(getCellHeaders(clazz), dataSet, new FileOutputStream(file));
    ...
    public static String[] getCellHeaders(Class<?> clazz) {
        Field[] fields = clazz.getDeclaredFields();
        return Stream.of(fields)
    ...
    public static byte[] file2Byte(File file) {
        ...
        try (FileInputStream fis = new FileInputStream(file);
            ...
            throw new RuntimeException("convert file to byte error!", e);
        }
        return buffer;
    }
    ...
    private static <T> void setCellValue(BufferedWriter bw, Field field, T row) throws Exception {
        ...
        if (value instanceof Date) {
            ...
            textValue = DateUtil.format(date, getFormatter(field, dateTimeFormatter));
